...
class FSM:
    def __init__(self, comm: AsyncCommunicator, peers: dict[str, Peer], rounds: int):
        ...
        self.state = "working"
        self.handlers = {
            "working": self.working_handler,
            "waiting": self.waiting_handler,
        }
        self.active = True
        ...
        self.comm.register_message_handler(MessageType.FSM, self.message_handler)
    ...
    async def working_handler(self) -> str:
        # Do some fake work
    ...
    async def waiting_handler(self) -> str:
    ...
    async def message_handler(self, sender: Peer, content: str, _: str):
        if content == "done":
